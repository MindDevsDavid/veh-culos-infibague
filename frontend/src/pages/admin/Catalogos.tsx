...
interface TipoCompItem { id: number; nombre_componente: string; descripcion_componente?: string | null }

const TABS = ['Marcas', 'Colores', 'Tipos Vehículo', 'Tipos Requisito', 'Tipos Componente', 'Dependencias'] as const
...
const TAB_KEY: Record<Tab, string> = {
  ...
  'Tipos Componente':   'tiposComponente',
...
const TAB_PATH: Record<Tab, string> = {
  ...
  'Tipos Componente':   '/catalogos/tipos-componente',
...
function CatalogoSection({ tab }: { tab: Tab }) {
  ...
  const isComp = tab === 'Tipos Componente'

  const { data: items = [], isLoading } = useQuery<(DepItem | TipoCompItem)[]>({
    queryKey: [qKey],
  ...
  function openCreate() { setEditing(null); setDesc(''); setDepIg(''); setNombre(''); setDescComp(''); setModal('create') }
  function openEdit(item: any) {
    setEditing(item)
    if (isComp) { setNombre(item.nombre_componente); setDescComp(item.descripcion_componente ?? '') }
    else { setDesc(item.descripcion); setDepIg(String(item.id_dependencia_ig ?? '')) }
    setModal('edit')
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (isComp) {
      ...
      payload = { nombre_componente: nombre.trim(), descripcion_componente: descComp.trim() || null }
    } else {
      if (!desc.trim()) return
      payload = { descripcion: desc.trim() }
  ...
  function itemLabel(item: any) {
    if (isComp) return item.nombre_componente
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={openCreate} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-3 py-1.5 rounded-lg transition-colors">
          <Plus size={14} /> Nuevo
        </button>
      </div>
        ...
        <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
          {items.map((item: any) => (
            <div key={item.id} className="flex items-center justify-between px-4 py-3 bg-white hover:bg-slate-50 transition-colors">
              <div>
                <span className="text-sm font-medium text-slate-800">{itemLabel(item)}</span>
                {isComp && item.descripcion_componente && (
                  <p className="text-xs text-slate-400 mt-0.5">{item.descripcion_componente}</p>
                )}
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => { if (confirm(`¿Eliminar "${itemLabel(item)}"?`)) remove.mutate(item.id) }}
                  className="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={modal !== null} onClose={() => setModal(null)} title={modal === 'create' ? `Nuevo — ${tab}` : `Editar — ${tab}`}>
        <form onSubmit={handleSubmit} className="space-y-4">
          {isComp ? (
            <>
              <FormField label="Nombre del componente" required value={nombre} onChange={e => setNombre(e.target.value)} />
              <FormField label="Descripción (opcional)" value={descComp} onChange={e => setDescComp(e.target.value)} />
            </>
          ) : (
            <>
              <FormField label="Descripción" required value={desc} onChange={e => setDesc(e.target.value)} />
              {isDep && <FormField label="ID Dependencia IG (opcional)" type="number" value={depIg} onChange={e => setDepIg(e.target.value)} />}
            </>
          )}

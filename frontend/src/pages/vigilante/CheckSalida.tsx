...
export default function CheckSalida() {
  ...
  const checkSalida = useCheckSalida()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await checkSalida.mutateAsync({ id: salidaId, form: fd })
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-800">Check Salida</h1>
        <p className="text-sm text-slate-500">Registrar hora exacta de salida del vehículo</p>
      </div>
      ...
      {!['AUTORIZADA','INSPECCIONADA'].includes(salida.estado) && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-800">
          Esta salida está en estado <strong>{salida.estado}</strong> — no se puede hacer check-salida.
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-3">
            <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
              <Camera size={16} /> Fotos del vehículo (máx. 2)
            </div>
            <input
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800">
            Al confirmar se registrará la hora exacta actual como hora de salida.
          </div>

          <button type="submit" disabled={submitting || checkSalida.isPending} className="w-full py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 disabled:opacity-60 transition-colors">
